...
class SSDShuffleNetV2FeatureExtractor(ssd_meta_arch.SSDFeatureExtractor):
  ...
  def __init__(self,
    ...
    """ShuffleNetV2 Feature Extractor for SSD Models.
  ...
  def extract_features(self, preprocessed_inputs):
    ...
    feature_map_layout = {
        'from_layer': ['Conv2d_13_shuffleblock', 'Conv2d_17_shuffleblock', '', '',
                       '', ''][:self._num_layers],
        'layer_depth': [-1, -1, 192, 96, 96, 48][:self._num_layers],
    ...
    with tf.variable_scope('ShufflenetV2',
      ...
      with slim.arg_scope(
        shufflenet_v2.shufflenet_v2_arg_scope(
              is_training=None, regularize_depthwise=True)):
        with (slim.arg_scope(self._conv_hyperparams_fn())
              if self._override_base_feature_extractor_hyperparams
              else context_manager.IdentityContextManager()):
          _, image_features = shufflenet_v2.shufflenet_v2_base(
              ops.pad_to_multiple(preprocessed_inputs, self._pad_to_multiple),
              final_endpoint='Conv2d_17_shuffleblock',
